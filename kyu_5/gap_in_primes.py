...
#
# In the example above gap(2, 3, 50) will return
# [3, 5] or (3, 5) or {3, 5} which is the first pair between 3 and 50 with a 2-gap.
#
...
# but 101-107is not a 6-gap because there is 103 in between and 103-109 is not a 6-gap because there is 107 in between.
#
...
#     return True
#
# Nawet z takim cudem z Wikipedii (Primality test) jest TimedOut


def gap(g, m, n):
    primes = [x for x in range(m, n + 1) if is_prime(x) == True]
    length_minus = len(primes) - 1
    for index, prime in enumerate(primes):
        next_index = index + 1
        if next_index > length_minus:
            return None
        if g == (primes[next_index] - primes[index]):
            return [primes[index], primes[next_index]]
